...
namespace ExcelConvertTools2
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            var configCheckBoxTable = ExcelOpenXml.GetSheet(configFile, "显示collectionoffice集合");
            if (configCheckBoxTable == null)
            {
                MessageBox.Show(@"列转换配置表[Config][显示collectionoffice集合]表数据不完整,请给出正确格式的配置文件");
                return;
            }

            foreach (var item in configCheckBoxTable.Columns.Cast<DataColumn>().Select(x => x.ColumnName))
            {
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            _dataTable = ExcelOpenXml.GetSheet(_fileName, "Sheet1", 3);
            ...
            for (int i = 0; i < _dataTable.Rows.Count; i++)
                if (_dataTable.Rows[i]["Incl. OFT."].ToString().Trim().ToUpper() == "N" && _configCollectionOfficeCheckedList.Contains(_dataTable.Rows[i]["Collection Office"].ToString().Trim()))
